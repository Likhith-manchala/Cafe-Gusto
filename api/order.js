...
export default async function handler(request, response) {
  ...
  const { customer_name, total_amount, payment_method, payment_reference, items } = request.body || {};
  if (!customer_name || !total_amount || !Array.isArray(items) || !items.length || !['upi', 'card', 'cash'].includes(payment_method)) {
  ...
  try {
    ...
    await sql`INSERT INTO orders (customer_name, table_number, total_amount, payment_method, payment_reference, payment_status, transaction_reference, items) VALUES (${customer_name}, ${tableNumber}, ${Number(total_amount)}, ${payment_method}, ${payment_reference || null}, ${paymentStatus}, ${transactionReference}, ${JSON.stringify(items)})`;
    return response.status(200).json({ success: true, table_number: tableNumber, transaction_reference: transactionReference, payment_status: paymentStatus });
  } catch (error) {
    console.error(error);
